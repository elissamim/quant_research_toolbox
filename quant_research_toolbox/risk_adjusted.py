...
def sharpe_ratio(returns:pd.Series,
                risk_free_rate:Optional[float]=0.0) -> float:
    """
    """
...
def sortino_ratio(returns:pd.Series,
    ...
    """
    """
    downside=DownsideRisk.downside_standard_deviation(returns,
...
def omega_ratio(returns:pd.Series,
               threshold:Optional[float] = 0.0) -> float:
    """
    """
    excess_gains = returns[returns > threshold] - threshold
    excess_losses = threshold - returns[returns < threshold]
    return float(excess_gains.sum()/excess_losses.sum()) if excess_losses.sum() > 0 else np.inf
